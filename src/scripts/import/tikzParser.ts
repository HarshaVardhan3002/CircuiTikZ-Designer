 ...
 */
export interface TikzPathControlsSegment extends BaseNode {
	kind: "controls"
	c1: TikzCoordinate
	c2: TikzCoordinate
}
...
class TikzParser {
	 ...
	 */
	private parseControlsSegment(intro: TikzToken): TikzPathControlsSegment | null {
		this.advance() // consume the first `..`

		// `controls` is the only legal continuation of `..` for the syntax we support.
		const ctrlTok = this.peek()
		if (!(ctrlTok.type === "IDENTIFIER" && ctrlTok.value === "controls")) {
			this.collector.warning("Expected 'controls' after '..' in path.", {
				line: intro.line,
				column: intro.column,
				code: "parse-controls-missing",
				suggestion: "The supported syntax is '.. controls (c1) and (c2) ..'.",
			})
			return null
		}
		this.advance()

		const c1Paren = this.check("LPAREN") ? this.parseParenElement() : null
		if (!c1Paren || c1Paren.kind !== "coord") {
			this.collector.warning("Couldn't parse first control point in '.. controls (.) and (.) ..'.", {
				line: intro.line,
				column: intro.column,
				code: "parse-controls-c1",
			})
			return null
		}

		// Optional `and` keyword between the two control points (TikZ requires it; we tolerate it
		// being missing because the parser used to ignore it before the bezier branch landed).
		if (this.peek().type === "IDENTIFIER" && this.peek().value === "and") {
			this.advance()
		}

		const c2Paren = this.check("LPAREN") ? this.parseParenElement() : null
		if (!c2Paren || c2Paren.kind !== "coord") {
			this.collector.warning("Couldn't parse second control point in '.. controls (.) and (.) ..'.", {
				line: intro.line,
				column: intro.column,
				code: "parse-controls-c2",
			})
			return null
		}
