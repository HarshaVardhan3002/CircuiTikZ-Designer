 ...
 */
export type ImportFormat = "auto" | "json" | "tikz" | "image"
...
export class ImportController {
	...
	/**
	 * Programmatic import of a CircuiTikZ or JSON string, with no modal / UI. The stable entry point
	 * for the harness / circuitAPI (MCP seam).
	 */
	public importString(text: string): void {
		const format = ImportController.detectFormat(text)
		if (format === "json") {
			this.importJSON(text)
		} else if (format === "tikz") {
			this.importTikZ(text)
		} else {
			throw new Error("Unrecognised import format (expected CircuiTikZ or JSON).")
		}
	}
