...
// --- Lexer: `..` becomes a DOTDOT token ---------------------------------------------------- //
//
// A bare `.` still tokenises as DOT; two of them in a row collapse to a single DOTDOT so the
// parser can recognise the `..controls..` connector cleanly.

summary("Lexer: '..' tokenizes as DOTDOT")
{
	const collector = new DiagnosticsCollector("\\draw (0,0) .. controls (1,2) and (2,2) .. (3,0);")
	const toks = tokenizeTikz(collector.sourceText, collector)
	const types = toks.map((t) => t.type)
	const dotdotCount = types.filter((t) => t === "DOTDOT").length
	assert(dotdotCount === 2, "Two `..` tokens picked up", `got ${dotdotCount}`)
	const stayDot = toks.find((t) => t.type === "DOT")
	assert(!stayDot, "No bare DOT tokens leak through", `got ${JSON.stringify(stayDot)}`)
}

summary("Lexer: a single '.' inside (1.5, 2) stays inside the NUMBER")
{
	const collector = new DiagnosticsCollector("(1.5, 2.0)")
	const toks = tokenizeTikz(collector.sourceText, collector)
	const numbers = toks.filter((t) => t.type === "NUMBER")
	assert(numbers.length === 2, "Two numbers", `got ${numbers.length}`)
	assert(numbers[0].parsed?.numeric === 1.5, "1.5 parses", `got ${numbers[0].parsed?.numeric}`)
	const dots = toks.filter((t) => t.type === "DOT" || t.type === "DOTDOT")
	assert(dots.length === 0, "No stray dot tokens emerge from numeric context")
}

// --- Parser: `..controls..` produces a controls path element ------------------------------- //

summary("Parser: '.. controls (c1) and (c2) .. (P1)' parses as a controls segment")
{
	const src = "\\draw (0,0) .. controls (1,2) and (2,2) .. (3,0);"
	const collector = new DiagnosticsCollector(src)
	const doc = parseTikz(src, collector)
	const drawStmt = doc.statements.find((s) => s.kind === "draw")
	assert(!!drawStmt, "Draw statement parsed")
	const controls = drawStmt.elements.filter((e) => e.kind === "controls")
	assert(controls.length === 1, "One controls segment", `got ${controls.length}`)
	assert(controls[0].c1.kind === "xy" && controls[0].c1.x === 1 && controls[0].c1.y === 2,
		"c1 parses as (1,2)", `got ${JSON.stringify(controls[0].c1)}`)
	assert(controls[0].c2.kind === "xy" && controls[0].c2.x === 2 && controls[0].c2.y === 2,
		"c2 parses as (2,2)", `got ${JSON.stringify(controls[0].c2)}`)
	const coords = drawStmt.elements.filter((e) => e.kind === "coord")
	assert(coords.length === 2, "Two coordinates flank the segment", `got ${coords.length}`)
}

summary("Parser: chained '.. controls (.) and (.) .. (.) .. controls (.) and (.) .. (.)'")
{
	const src =
	...
	const drawStmt = doc.statements.find((s) => s.kind === "draw")
	const controls = drawStmt.elements.filter((e) => e.kind === "controls")
	assert(controls.length === 2, "Two segments parsed", `got ${controls.length}`)
	const coords = drawStmt.elements.filter((e) => e.kind === "coord")
	assert(coords.length === 3, "Three coordinates", `got ${coords.length}`)
	const warnings = collector.all().filter((d) => d.severity === "warning")
	assert(warnings.length === 0, "No warnings on a clean fragment", `got ${JSON.stringify(warnings)}`)
}

summary("Parser: missing 'controls' after '..' → warning, not crash")
{
	const src = "\\draw (0,0) .. (3,0);"
	const collector = new DiagnosticsCollector(src)
	const doc = parseTikz(src, collector)
	assert(doc.statements.length >= 1, "Draw statement survives the bad fragment")
	const codes = collector.all().map((d) => d.code)
	assert(codes.includes("parse-controls-missing"), "Emits parse-controls-missing", `codes=${codes.join(",")}`)
}

summary("Parser: missing trailing '..' → warning, segment still produced")
{
	const src = "\\draw (0,0) .. controls (1,2) and (2,2) (3,0);"
	const collector = new DiagnosticsCollector(src)
	const doc = parseTikz(src, collector)
	const drawStmt = doc.statements.find((s) => s.kind === "draw")
	const controls = drawStmt.elements.filter((e) => e.kind === "controls")
	assert(controls.length === 1, "Segment is still emitted", `got ${controls.length}`)
	const codes = collector.all().map((d) => d.code)
	assert(codes.includes("parse-controls-trailing"), "Emits parse-controls-trailing")
}
...
summary("Parser: stress - 20-segment alternating spline+wire mix doesn't choke")
{
	...
	for (let i = 1; i <= 20; i++) {
		if (i % 2 === 0) {
			src += ` .. controls (${i - 0.5},${i + 1}) and (${i - 0.3},${i - 1}) .. (${i},0)`
		} else {
			src += ` -- (${i},0)`
		}
	}
	...
	const drawStmt = doc.statements.find((s) => s.kind === "draw")
	const controls = drawStmt.elements.filter((e) => e.kind === "controls")
	assert(controls.length === 10, "All 10 controls segments parsed", `got ${controls.length}`)
	const conns = drawStmt.elements.filter((e) => e.kind === "connector")
	assert(conns.length === 10, "All 10 wire connectors parsed", `got ${conns.length}`)
	const errors = collector.all().filter((d) => d.severity === "error")
	assert(errors.length === 0, "No errors", `got ${JSON.stringify(errors)}`)
}
