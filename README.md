...
An interactive, visual, online editor for creating electrical circuit diagrams for LaTeX/CircuiTi*k*Z with ease — now with AI Beta circuit detection from a photo of a hand-drawn diagram. Try it out [online](https://circuit2tikz.tf.fau.de/designer/)!

## What's new in v1.1

CircuiTi*k*Z-Designer v1.1 makes the path from "I have a circuit on paper" to "compilable LaTeX" dramatically shorter:

- **Detect from image (AI Beta)** — drop a photo or scan of a hand-drawn circuit and let a configured vision model parse it into editable components and wires. Bring your own provider; nothing leaves your machine without your key.
...
- **First-class top-bar** — Save, Import, Export, Undo / Redo and Settings are visible without searching. The Import dropdown surfaces "Detect from image (AI Beta)" with a sparkle icon next to "From file" and "Paste TikZ".
...
### Beta scope for maintainers

The following v1.1 surfaces are intentionally marked **Beta** because their quality depends on the selected provider/model and user review:

- Detect from image / image-to-circuit import
- AI Provider settings and provider-status chips
- AI Assistant chat, visual check, and model tool-calling
- MCP / harness automation flows intended for model-driven editing

The core editor, modern UI, command palette, parser/import diagnostics, spline editor, JSON/TikZ import, and export paths are not labelled Beta.
...
- AI-assisted intake from a hand-drawn sketch (Beta) — bring your own provider, your keys never leave the browser
...
- **Detect from Image (AI Beta):** upload a hand-drawn circuit photo; the app extracts components and wires using a vision LLM you configure (OpenAI / Anthropic / Gemini / local Ollama / LM Studio). Per-component confidence flagging lets you review before accepting.
...
To use **Detect from image (AI Beta)**:

1. Open **Settings** (gear icon, top right) and configure an AI Beta provider in the *AI Provider* section. You'll need an API key for OpenAI, Anthropic, or Google — or a base URL to your local Ollama / LM Studio / vLLM endpoint.
2. Click **Import → Detect from image (AI Beta)** in the navbar (or just drag an image file onto the canvas).
...
These are aspirations — not commitments. Highlights:
...
- **V2+ — MCP Integration.** Expose the editor and detection pipeline as a Model Context Protocol server so external automation tools can inspect, edit, and extend circuits programmatically.
